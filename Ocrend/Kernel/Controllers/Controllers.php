...
abstract class Controllers {
    ...
    */
    ...
    private function setControllerConfig($config) {
      ...
      # Configuraciones de los roles
      $this->controllerConfig['users_admins'] = false;
      $this->controllerConfig['users_normales'] = false;
      ...
      if (null != $config) {
        ...
        # Configura el controlador para solo ser visto por usuarios admins
        if (array_key_exists('users_admins', $config)) {
          $this->controllerConfig['users_admins'] = (bool) $config['users_admins'];
        }
        # Configura el controlador para solo ser visto por usuarios normales
        if (array_key_exists('users_normales', $config)) {
          $this->controllerConfig['users_normales'] = (bool) $config['users_normales'];
        }
      }
    }
    ...
    private function knowVisitorPermissions() {
      ...
      if($this->is_logged) {
        # Si es para admin, y es normal, se redireccion
        if($this->controllerConfig['users_admins'] && $this->user['rol'] == 0) {
          $this->functions->redir();
        }

        # Si es para normales, y es admin, se redirecciona
        if($this->controllerConfig['users_normales'] && $this->user['rol'] == 1) {
          $this->functions->redir();
        }
      }
    }
